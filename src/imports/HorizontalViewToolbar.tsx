...
function ToolbarTextLabel({ isActive, onClick, isExpanded }: { isActive: boolean; onClick: () => void; isExpanded?: boolean }) {
  return (
    <motion.div 
      className={`${isActive ? 'bg-[#DFF5FC]' : ''} flex ${isExpanded ? 'flex-col' : 'flex-row'} items-center ${isExpanded ? 'py-[8px]' : ''} px-0 gap-[4px] ${isExpanded ? 'w-[60px]' : 'h-[60px]'} relative rounded-[4px] shrink-0 cursor-pointer`} 
