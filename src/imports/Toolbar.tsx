...
function ToolbarTextLabel({ isActive, onClick, microAnimations = true }: { isActive: boolean; onClick: () => void; microAnimations?: boolean }) {
  ...
  return (
    <motion.div 
      className={`${isActive ? 'bg-[#DFF5FC]' : ''} flex flex-row items-center px-[8px] py-0 gap-[4px] h-[60px] relative rounded-[4px] shrink-0 cursor-pointer self-stretch`} 
...
function ToolbarTextLabel1({ isActive, onClick, microAnimations = true }: { isActive: boolean; onClick: () => void; microAnimations?: boolean }) {
  const animationProps = microAnimations ? {
    animate: {
      scale: isActive ? 1.08 : 1,
    },
...
function ToolbarTextLabel2({ isActive, onClick, microAnimations = true }: { isActive: boolean; onClick: () => void; microAnimations?: boolean }) {
  const animationProps = microAnimations ? {
    animate: {
      scale: isActive ? 1.08 : 1,
    },
...
function Frame5({ activeButtons, onButtonClick, microAnimations = true }: { activeButtons: Set<number>; onButtonClick: (index: number) => void; microAnimations?: boolean }) {
  return (
    <div className="box-border content-stretch flex flex-col gap-[8px] items-stretch p-[8px] relative shrink-0 w-full">
      <ToolbarTextLabel isActive={activeButtons.has(0)} onClick={() => onButtonClick(0)} microAnimations={microAnimations} />
      <ToolbarTextLabel1 isActive={activeButtons.has(1)} onClick={() => onButtonClick(1)} microAnimations={microAnimations} />
      <ToolbarTextLabel2 isActive={activeButtons.has(2)} onClick={() => onButtonClick(2)} microAnimations={microAnimations} />
    </div>
  );
}
...
function Frame4({ onButtonClick, activeButtons }: { onButtonClick: (index: number) => void; activeButtons: Set<number> }) {
  return (
    <div className="box-border flex flex-col items-center px-[8px] pb-[8px] pt-0 gap-[8px] w-full border-t border-[rgba(0,0,0,0.1)] rounded-bl-[12px] rounded-br-[12px] relative shrink-0">
      <AohsButton3 onClick={() => onButtonClick(3)} isExpanded={activeButtons.has(3)} />
    </div>
  );
}
